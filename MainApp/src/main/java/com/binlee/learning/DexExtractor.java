...
final class DexExtractor implements Closeable {
  ...
  /** We look for all dex files named {@code *.dex} */
  ...
  private static final String PREFS_FILE = "plugin.version";
  ...
  DexExtractor(File sourceApk, File dexDir) throws IOException {
    ...
    File lockFile = new File(dexDir, LOCK_FILENAME/*PluginDex.lock*/);
  ...
  List<? extends File> load(Context context, String prefsKeyPrefix, boolean forceReload)
    throws IOException {
    Log.i(TAG, "DexExtractor.load(" + sourceApk.getPath() + ", " + forceReload + ", " +
      prefsKeyPrefix + ")");
    ...
    if (!forceReload && !isModified(context, sourceApk, sourceCrc, prefsKeyPrefix)) {
      try {
        ...
        putStoredApkInfo(context, prefsKeyPrefix, getTimeStamp(sourceApk), sourceCrc, files);
      }
    } else {
      if (forceReload) {
  ...
  private List<ExtractedDex> loadExistingExtractions(Context context, String prefsKeyPrefix) throws IOException {
    ...
    SharedPreferences multiDexPreferences = getPluginDexPreferences(context);
    ...
    for (int secondaryNumber = 2; secondaryNumber <= totalDexNumber; secondaryNumber++) {
      ...
      if (extractedFile.isFile()) {
        ...
        throw new IOException("Missing extracted plugin dex file '" + extractedFile.getPath() + "'");
      }
    }
  ...
  private static boolean isModified(Context context, File archive, long currentCrc,
    ...
    SharedPreferences prefs = getPluginDexPreferences(context);
  ...
  private List<ExtractedDex> performExtractions() throws IOException {
    ...
    try {
      ...
      for (int i = 1; ; i++) {
        ...
        // classes{}.dex
        dexFile = apk.getEntry(DEX_PREFIX + index + DEX_SUFFIX);
        if (dexFile == null) break;
        // com.sample.plugin.apk.classes{num}.zip
        String fileName = extractedFilePrefix + index + EXTRACTED_SUFFIX;
        ExtractedDex extractedFile = new ExtractedDex(dexDir, fileName);
        files.add(extractedFile);
  ...
  /**
   * Get the PluginDex {@link SharedPreferences} for the current application. Should be called only
   * while owning the lock on {@link #LOCK_FILENAME}.
   */
  private static SharedPreferences getPluginDexPreferences(Context context) {
