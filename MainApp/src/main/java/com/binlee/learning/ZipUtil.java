...
final class ZipUtil {
  ...
  static long getZipCrc(File apk) throws IOException {
    try (RandomAccessFile raf = new RandomAccessFile(apk, "r")) {
      Pair<Long, Long> dir = findCentralDirectory(raf);
      return computeCrcOfCentralDir(raf, dir);
    }
  }

  /* Package visible for testing */
  static Pair<Long, Long> findCentralDirectory(RandomAccessFile raf) throws IOException, ZipException {
    ...
    return Pair.create(/*size=*/Integer.reverseBytes(raf.readInt()) & 0xFFFFFFFFL,
      /*offset=*/Integer.reverseBytes(raf.readInt()) & 0xFFFFFFFFL);
  }

  /* Package visible for testing */
  static long computeCrcOfCentralDir(RandomAccessFile raf, Pair<Long, Long> dir) throws IOException {
    CRC32 crc = new CRC32();
    long stillToRead = dir.first;
    raf.seek(dir.second);
