...
public class Camera2Helper {
    ...
    //{#see mCaptureCallback}The current state of camera state for taking pictures.
    private int mState = STATE_PREVIEW;
    private CameraManager manager;
    private AfterDoListener mAfterDoListener;
    ...
    // 注意:用volatile修饰的变量，线程在每次使用变量的时候，都会读取变量修改后的最的值
    private volatile static Camera2Helper singleton;

    private Camera2Helper(AutoFitTextureView view) {
        textureView = view;
    }

    private Camera2Helper(AutoFitTextureView view, File file) {
        textureView = view;
        mFile = file;
    ...
    public static Camera2Helper getInstance(AutoFitTextureView view, File file) {
        if (singleton == null) {
            synchronized (Camera2Helper.class) {
                singleton = new Camera2Helper(view, file);
            }
        }
        return singleton;
    }
        ...
        switch (mState) {
            case STATE_PREVIEW: {
                if (isNeedHideProgressbar) {
                    mAfterDoListener.onAfterPreviewBack();
        ...
        try {
            ...
            CameraCaptureSession.CaptureCallback CaptureCallback = new CameraCaptureSession.CaptureCallback() {
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                                               ...
                                               @NonNull TotalCaptureResult result) {
                    unlockFocus();
                    mAfterDoListener.onAfterTakePicture();
    ...
    private void setUpCameraOutputs(int width, int height) {
        try {
            ...
            for (String cameraId : cameraIdList) {
                ...
                int orientation = textureView.getContext().getResources().getConfiguration().orientation;
...
=======
  ...
  private void openCamera(int width, int height) {
    ...
    try {
      ...
      if (ActivityCompat.checkSelfPermission(textureView.getContext(), Manifest.permission.CAMERA)
  ...
  private static class ImageSaver implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        //压缩
        Bitmap bitmapAfter = BitmapUtil.compressBitmapBySampleSize(bitmap, 4);
        //压缩后的Bitmap 转为Bytes
        byte[] bytesAfter = BitmapUtil.bitmap2Bytes(bitmapAfter);
        output = new FileOutputStream(mFile);
        //写到文件输出流
        output.write(bytesAfter);
      } catch (IOException e) {
        LogUtil.showErroLog(e.getMessage());
        e.printStackTrace();
      } finally {
        mImage.close();
        if (null != output) {
...
>>>>>>> 21841b3 (feat: save camera picture size & preview size to shared preference file)
            e.printStackTrace();
        } catch (NullPointerException e) {
        }
    }
