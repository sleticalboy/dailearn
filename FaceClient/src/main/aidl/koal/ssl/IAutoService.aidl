...
interface IAutoService {
	...
	*/
	...
	[IN]port：服务器端口
	*/
	void setServerAddr(in String ip, in String port);
	...
	*/
	...
	返回值：0 = 未链接，1=已链接
	*/
	int getAppStatus(in String appName);
	...
	[IN]save： 是否保存PIN码
	*/
	void setPin(in String pin, in boolean save);
	...
	*/
	void setCheckAppInterval(in int seconds);
		
	/**
	设置显示toast消息
	[IN]display： true=显示；false=不显示
	*/
	void setDisplayToast(in boolean display);
	...
	*/
	...
	设置当前证书（默认为第一张证书）
	*/
	void setCurCert(in String cert);
	
	/**
	使用PIN码检查设备
	*/
	void checkDevice(in String pin);
	
	/**
	设置TF卡型号
	*/
	void setTFModel(in String tf);
	...
	*/
	String getConfig(in String key);
	...
	*/
	String getDevInfo(in String key);
	...
	*/
	String getServiceInfo(in String key);
}
